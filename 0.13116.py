...
def single_model(clf, train_x, train_y, test_x, clf_name, class_num=1):
    ...
    return val_pred, test_pred, mean_squared_error(val_y, val_pred)


def lgb_model(x_train, y_train, x_valid):
    lgb_train, lgb_test, lgb_mse = single_model(lgb, x_train, y_train, x_valid, "lgb", 1)
    return lgb_train, lgb_test, lgb_mse

def rf_model(x_train, y_train, x_valid):
    rf_train, rf_test, rf_mse = single_model(rf, x_train, y_train, x_valid, 'rf', 1)
    return rf_train, rf_test, rf_mse

def xgb_model(x_train, y_train, x_valid):
    xgb_train, xgb_test, xgb_mse = single_model(xgb, x_train, y_train, x_valid, "xgb", 1)
    return xgb_train, xgb_test, xgb_mse


def cat_model(x_train, y_train, x_valid):
    cat_train, cat_test, cat_mse = single_model(CatBoostRegressor, x_train, y_train, x_valid, "cat", 1)
    return cat_train, cat_test, cat_mse


def sgd_model(x_train, y_train, x_valid):
    sgd_train, sgd_test, sgd_mse = single_model(SGDRegressor, x_train, y_train, x_valid, "sgd", 1)
    return sgd_train, sgd_test, sgd_mse


def ridge_model(x_train, y_train, x_valid):
    ridge_train, ridge_test, ridge_mse = single_model(Ridge, x_train, y_train, x_valid, "ridge", 1)
    return ridge_train, ridge_test, ridge_mse
...
# y_val = y_train[y_train.shape[0] * 0.80:]

rf_train, rf_test, rf_mse = rf_model(x_train, y_train, x_test)

ridge_train, ridge_test, ridge_mse  = ridge_model(x_train, y_train, x_test)

sgd_train, sgd_test, sgd_mse = sgd_model(x_train, y_train, x_test)

lgb_train, lgb_test, lgb_mse = lgb_model(x_train, y_train, x_test)

xgb_train, xgb_test, xgb_mse = xgb_model(x_train, y_train, x_test)

cat_train, cat_test, cat_mse = cat_model(x_train, y_train, x_test)


# train_pred = (rf_train + lr_train + sgd_train + lgb_train[:, 0] + xgb_train[:, 0] + cat_train) / 6
test_pred = (rf_mse*rf_test + ridge_mse*ridge_test + sgd_mse*sgd_test + lgb_mse*lgb_test[:, 0] + xgb_mse*xgb_test[:, 0] + cat_mse*cat_test) / (rf_mse+ridge_mse+sgd_mse+lgb_mse+xgb_mse+cat_mse)
